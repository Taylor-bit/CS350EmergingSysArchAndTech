...
class TemperatureMachine(StateMachine):
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            firstLine = current_time.strftime("%m%d %H:%M:%S")

            ## Setup Display Line 2
            if(altCounter < 6):
                secondLine = f"{floor(self.getFahrenheit())}F"
                altCounter = altCounter + 1
            else:
                secondLine = f"{self.current_state.id} Set: {self.setPoint}"
                altCounter = altCounter + 1
                if(altCounter >= 11):
            ...
            ## Update Display
            screen.updateScreen(f"{firstLine}\n{secondLine}")
    
            ## Update server every 30 seconds
            if(DEBUG):
               print(f"Counter: {counter}")
            if((counter % 30) == 0):
                ser.write((f"{self.setupSerialOutput()}\n").encode("utf-8"))
                counter = 1
            else:
                counter = counter + 1
            sleep(1)
